...
export class RecordRepo {
    ...
    async search(request: SearchReq): Promise<RecordEntity[]> {
        ...
        let result: { rc: RecordEntity[] } = { rc: [] };

        let startDate = moment(request.startDate);
        let endDate = moment(request.endDate);
        ...
        let whereFunc = `function () {  const arrSum = arr => arr.reduce((a, b) => a + b, 0);
